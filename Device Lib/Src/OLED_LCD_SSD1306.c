...
static void SSD1306_I2C_WriteMultiByte(SSD1306_Name* SSD1306, uint8_t address, uint8_t reg, uint8_t* data, uint16_t count)
{
	uint8_t TxBuff[256];
	TxBuff[0] = reg;
	uint8_t i;
...
static void SSD1306_I2C_WriteOneByte(SSD1306_Name* SSD1306, uint8_t address, uint8_t reg, uint8_t data)
{
	uint8_t TxBuff[2];
	TxBuff[0] = reg;
	TxBuff[1] = data;
	HAL_I2C_Master_Transmit(SSD1306->I2C, address, TxBuff, 2, 10);
}

static void SSD1306_WriteCmd(SSD1306_Name* SSD1306, uint8_t Cmd)
{
	SSD1306_I2C_WriteOneByte(SSD1306, SSD1306->Address, SSD1306_SEND_CMD, Cmd);
}
...
{
	SSD1306->I2C = I2C_In;
	SSD1306->Address = SSD1306_I2C_ADDR;
	if (HAL_I2C_IsDeviceReady(SSD1306->I2C, SSD1306->Address, 1, 20000) != HAL_OK) 
	{
		/* Return false */
		return 0;
	}
	...
	while(p>0) p--;
	/* Init LCD */
	SSD1306_WriteCmd(SSD1306, SSD1306_DISPLAY_OFF); //display off
	...
	SSD1306_WriteCmd(SSD1306, 0x22); // Set Page Address
	...
	SSD1306_WriteCmd(SSD1306, SSD1306_DISPLAY_ON); //--turn on SSD1306 panel
...
void SSD1306_UpdateScreen(SSD1306_Name* SSD1306) 
{
	for (uint8_t m = 0; m < 8; m++)
	{
		...
		/* Write data to Ram */
		SSD1306_I2C_WriteMultiByte(SSD1306, SSD1306_I2C_ADDR, SSD1306_SEND_DATA, &SSD1306->Buffer[SSD1306_WIDTH * m], SSD1306_WIDTH);
	}
}
...
void SSD1306_InvertDisplay (SSD1306_Name* SSD1306, int Invert)
{
  if(Invert) 
	{
		SSD1306_WriteCmd(SSD1306, SSD1306_INVERTDISPLAY);
	}
  else
	{		
		SSD1306_WriteCmd(SSD1306, SSD1306_NORMALDISPLAY);
	}
}
...
char SSD1306_Putc(SSD1306_Name* SSD1306, char ch, FontDef_t* Font, SSD1306_COLOR_t color) 
{
	...
	{
		Data_Byte = Font->data[(ch - 32) * Font->FontHeight + i];
		for (int j = 0; j < Font->FontWidth; j++) 
		{
			if ((Data_Byte << j) & 0x8000) // send msb
			{
