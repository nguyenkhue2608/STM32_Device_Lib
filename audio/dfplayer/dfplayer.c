/**
 * @file    dfplayer.c
 * @brief   DFPlayer Mini MP3 module, UART - implementation.
 *
 * @author  Khue Nguyen
...
//******************************** LOW Level Functions ****************************//
static void DFPLAYER_SendUART(DFPLAYER_Device* MP3)
{
...
static void DFPLAYER_FillChecksum (DFPLAYER_Device* MP3){
	uint16_t checksum = DFPLAYER_GetChecksum (MP3->SendBuff);
	DFPLAYER_FillBigend (MP3->SendBuff+7, checksum);
}

static void DFPLAYER_SendCmd(DFPLAYER_Device* MP3, uint8_t cmd, uint16_t high_arg, uint16_t low_arg)
{
	DFPLAYER_Delay(100);
	MP3->SendBuff[3] = cmd;
